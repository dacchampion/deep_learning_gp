...
def parse_args():
    ...
    arg_parser.add_argument('--name', required=True, help='Image name (without suffix)')
    arg_parser.add_argument('--imdir', required=True, help='Image directory')
    arg_parser.add_argument('--csv', required=True, help='Output csv file')
    arg_parser.add_argument('--to-plot', action='store_true', help='Plots extracted contours')
...
def expand_polygon(poly, expansion_pts=POLYGON_EXPANSION_PIXELS):
    ...
    center = [x for xs in center for x in xs]
    new_coords = []
